...
function agregar_pagina_de_opciones() {
  add_menu_page( 'Mindestec Fitness', 'MFitness', 'manage_options', 'opciones_de_mi_plugin', null, plugins_url('../assets/Logo.svg', __FILE__) );
  add_submenu_page( 'opciones_de_mi_plugin', 'Colores del Gráfico', 'Colores del Gráfico', 'manage_options', 'opciones_de_mi_plugin', 'generar_pagina_de_opciones');
  add_submenu_page('opciones_de_mi_plugin', 'Exportar Usuarios', 'Exportar Usuarios', 'manage_options', 'exportar_datos_usuarios', 'exportar_datos_usuarios');
}

add_action( 'admin_menu', 'agregar_pagina_de_opciones' );
...
add_action('admin_post_exportar_usuarios', 'exportar_usuarios_csv');

function exportar_usuarios_csv() {
  try{
...
function exportar_datos_usuarios(){ ?>
	...
	<h1>Exportar Datos de los Alumnos</h1>
		<div class="wrap">
			...
			<form method="post" action="<?php echo esc_url(admin_url('admin-post.php?action=exportar_usuarios_csv')); ?>" id="exportar_usuarios_form">
...
function actualizar_ultimo_color_seleccionado() {
...
add_action( 'admin_init', 'actualizar_ultimo_color_seleccionado' );

// Función para generar la página de opciones
function generar_pagina_de_opciones() {
...
function agregar_opciones_de_color() {
...
add_action( 'admin_init', 'agregar_opciones_de_color' );

// Reglas de validación y sanitización para las opciones de color
function validar_opciones_de_color( $input ) {
...
add_filter( 'sanitize_option_opciones_de_mi_plugin', 'validar_opciones_de_color' );
